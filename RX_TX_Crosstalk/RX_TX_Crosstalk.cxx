// modified to show RX LO interference on the B200
// modifications by M. H. Reilly kb1vc 
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    size_t spb;
    double rate, freq, rx_freq, gain, bw;
    float ampl;
    ...
    desc.add_options()
        ...
        ("rx-freq", po::value<double>(&rx_freq), "Receiver RF center frequency in Hz -- for interference tests")
	...
	uhd::tune_result_t tres; 
        ...
        tres = usrp->set_tx_freq(tune_request, channel_nums[ch]);
	...
	if(do_rx) {
	  ...
	  uhd::tune_result_t tres; 
	  if(vm.count("int-n")) tune_request.args = uhd::device_addr_t("mode_n=integer");
	  tres = usrp->set_rx_freq(tune_request, channel_nums[ch]);
	  std::cout << boost::format("Actual RX Freq: %f MHz...") % (usrp->get_rx_freq(channel_nums[ch])/1e6) << std::endl << std::endl;
	  std::cout << boost::format("\tRF_actual = %16.14g MHz DDC = %g Hz\n")
	    % (tres.actual_rf_freq * 1e-6) % tres.actual_dsp_freq;
	}
